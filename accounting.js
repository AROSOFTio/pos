...
export function registerAccountingRoutes(app,{pool,auth,tenant,getBiz,permit,rolesAllowed,audit}){
  ...
  app.get('/api/accounting/dashboard',auth,tenant,permit('accounting.view'),async(req,res)=>{const bid=await getBiz(req),from=parseDate(req.query.from),to=parseDate(req.query.to),branchId=Number(req.query.branchId||0)||null;const [pnl,tb,cash,journals,ap,ar]=await Promise.all([
    ...
    pool.query("SELECT a.account_code,a.name,coalesce(sum(CASE WHEN je.status='posted' THEN jl.debit-jl.credit ELSE 0 END),0)::numeric balance FROM accounts a LEFT JOIN journal_lines jl ON jl.account_id=a.id LEFT JOIN journal_entries je ON je.id=jl.journal_entry_id WHERE a.business_id=$1 AND a.account_code LIKE '111%' GROUP BY a.id ORDER BY a.account_code",[bid]),
    pool.query("SELECT count(*)::int count FROM journal_entries WHERE business_id=$1 AND status='posted' AND ($2::date IS NULL OR entry_date >= $2::date) AND ($3::date IS NULL OR entry_date <= $3::date)",[bid,from,to]),
  ...
  ]);const revenue=pnl.filter(x=>x.account_type==='revenue').reduce((n,x)=>n+x.credit-x.debit,0),cogs=pnl.filter(x=>x.account_type==='cost_of_sales').reduce((n,x)=>n+x.debit-x.credit,0),expenses=pnl.filter(x=>x.account_type==='expense').reduce((n,x)=>n+x.debit-x.credit,0),totalDr=tb.reduce((n,x)=>n+x.debit,0),totalCr=tb.reduce((n,x)=>n+x.credit,0);res.json({revenue:money(revenue),cogs:money(cogs),grossProfit:money(revenue-cogs),expenses:money(expenses),netProfit:money(revenue-cogs-expenses),accountsPayable:Number(ap.rows[0].v||0),accountsReceivable:Number(ar.rows[0].v||0),journalCount:journals.rows[0].count,trialBalance:{debit:money(totalDr),credit:money(totalCr),balanced:Math.abs(totalDr-totalCr)<=0.01},cash:cash.rows.map(x=>({...x,balance:Number(x.balance||0)}))})});

  app.get('/api/reports/management-dashboard',auth,tenant,permit('reports.profit'),async(req,res)=>{const bid=await getBiz(req),from=parseDate(req.query.from),to=parseDate(req.query.to),branchId=Number(req.query.branchId||0)||null,p=[bid,from,to,branchId];const [sales,orders,payments,refunds,expenses,purchases,inventory,production,kitchen,tables,customers,staff,trend,paymentMix,branches,shifts,top]=await Promise.all([
    pool.query("SELECT count(*)::int count,coalesce(sum(total),0)::numeric total,coalesce(sum(discount),0)::numeric discount,coalesce(sum(tax),0)::numeric tax FROM sales WHERE business_id=$1 AND voided=false AND ($2::date IS NULL OR created_at::date >= $2::date) AND ($3::date IS NULL OR created_at::date <= $3::date) AND ($4::bigint IS NULL OR branch_id=$4)",p),
    ...
    pool.query("SELECT count(*)::int count,coalesce(sum(total),0)::numeric total FROM refunds WHERE business_id=$1 AND status='approved' AND ($2::date IS NULL OR approved_at::date >= $2::date) AND ($3::date IS NULL OR approved_at::date <= $3::date)",[bid,from,to]),
    ...
    pool.query("SELECT cs.shift_no,cs.opened_by,coalesce(b.name,'-') branch,cs.status,cs.opened_at,cs.closed_at,coalesce(sum(p.amount),0)::numeric collected FROM cash_sessions cs LEFT JOIN branches b ON b.id=cs.branch_id LEFT JOIN payments p ON p.cash_session_id=cs.id AND p.status='posted' WHERE cs.business_id=$1 AND ($2::date IS NULL OR cs.opened_at::date >= $2::date) AND ($3::date IS NULL OR cs.opened_at::date <= $3::date) GROUP BY cs.id,b.name ORDER BY cs.id DESC LIMIT 30",[bid,from,to]),
  ...
  ]);res.json({sales:sales.rows[0],restaurant:orders.rows[0],payments:payments.rows[0],refunds:refunds.rows[0],expenses:expenses.rows[0],purchases:purchases.rows[0],inventory:inventory.rows[0],production:production.rows[0],kitchen:kitchen.rows[0],tables:tables.rows[0],customers:customers.rows[0],staff:staff.rows[0],trend:trend.rows.map(x=>({...x,sales:Number(x.sales),expenses:Number(x.expenses),refunds:Number(x.refunds)})),paymentMix:paymentMix.rows.map(x=>({...x,total:Number(x.total)})),branches:branches.rows.map(x=>({...x,sales:Number(x.sales)})),shifts:shifts.rows.map(x=>({...x,collected:Number(x.collected)})),topItems:top.rows.map(x=>({...x,qty:Number(x.qty),revenue:Number(x.revenue),cogs:Number(x.cogs),contribution:Number(x.revenue)-Number(x.cogs)}))})});
