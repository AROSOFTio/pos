...
export default function Register({onLogin,navigate}:{onLogin:(u:User)=>void;navigate:(path:string)=>void}){
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#172033]">
    ...
    <main className="mx-auto max-w-[700px] px-3 pb-10 pt-3 sm:px-5 sm:pt-6">
      <form onSubmit={submit} autoComplete="on" className="rounded-2xl border border-slate-200 bg-white p-4 premium-shadow sm:p-7">
        <div className="text-xs font-medium uppercase tracking-[.16em] text-[#22A53A]">14-day free trial</div>
        <h1 className="mt-2 text-[30px] font-semibold tracking-[-.04em]">Create workspace</h1>
        ...
        <button disabled={busy} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#22A53A] text-[14px] font-medium text-white disabled:opacity-50">{busy?'Creating…':'Start Free Trial'} {!busy&&<ArrowRight size={17}/>}</button>
      </form>
    </main>
  </div>
}
function Field({label,children}:{label:string;children:any}){return <label className="text-[13px] font-medium text-slate-600">{label}{children}</label>}
