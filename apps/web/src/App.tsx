...
function applyTheme(key:string,mode:string,customAccent='',background='clean',scope='operations'){
  ...
  root.dataset.backgroundScope=scope
}
...
function Login({onLogin,navigate}:{onLogin:(u:User)=>void;navigate:(path:string)=>void}){
  ...
  return <div className="min-h-screen bg-[#EEF2F5] text-[#172033]">
    <main className="flex min-h-screen items-center justify-center px-4 py-8 sm:px-6">
      <div className="w-full max-w-[420px]">
        ...
        {!forgot?<form onSubmit={submit} autoComplete="on" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,.06)] sm:p-7">
          <h1 className="text-[26px] font-semibold tracking-[-.035em] text-slate-950">Sign in</h1>
          <p className="mt-1 text-[12px] text-slate-500">Access your MauzoPOS workspace.</p>
          <label className="mt-6 block text-[11.5px] font-medium text-slate-700">Email address
            <input name="email" type="email" autoComplete="email" placeholder="you@example.com" required className="control mt-1.5"/>
          </label>
          <div className="mt-4 flex items-center justify-between">
            <label htmlFor="login-password" className="text-[11.5px] font-medium text-slate-700">Password</label>
            <button type="button" onClick={()=>{setForgot(true);setError('');setSent(false)}} className="text-[11px] font-medium text-[var(--brand-primary)] hover:underline">Forgot password?</button>
          </div>
          ...
          <button disabled={busy} className="mt-5 flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-[var(--brand-primary)] text-[13px] font-semibold text-white disabled:opacity-50">{busy?'Signing in…':'Sign in'} {!busy&&<ArrowRight size={15}/>}</button>
          <div className="mt-5 border-t border-slate-100 pt-4 text-center text-[11px] text-slate-500">New to MauzoPOS? <button type="button" onClick={()=>navigate('/register')} className="font-semibold text-[var(--brand-primary)] hover:underline">Start free trial</button></div>
        </form>:<form onSubmit={requestReset} autoComplete="on" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,.06)] sm:p-7">
          ...
          <h1 className="mt-5 text-[25px] font-semibold tracking-[-.035em] text-slate-950">Reset password</h1>
          <p className="mt-1 text-[12px] text-slate-500">Enter the email connected to your account.</p>
          <label className="mt-6 block text-[11.5px] font-medium text-slate-700">Email address
            <input name="email" type="email" autoComplete="email" placeholder="you@example.com" required className="control mt-1.5"/>
          </label>
          {sent&&<div className="mt-4 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2.5 text-[11px] font-medium text-emerald-700">If the account exists, reset instructions have been sent.</div>}
          {error&&<div className="mt-4 rounded-lg border border-red-100 bg-red-50 px-3 py-2.5 text-[11px] font-medium text-red-700">{error}</div>}
          <button disabled={busy} className="mt-5 h-11 w-full rounded-lg bg-slate-950 text-[13px] font-semibold text-white disabled:opacity-50">{busy?'Sending…':'Send reset link'}</button>
        </form>}
        <div className="mt-4 flex items-center justify-center gap-3 text-[10px] text-slate-400">
          <button onClick={()=>navigate('/')} className="hover:text-slate-600">Website</button><span>•</span><span>Secure access</span>
        </div>
      </div>
    </main>
  </div>
