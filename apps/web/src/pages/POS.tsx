 ...
 return <div>
  ...
  <div className="grid xl:grid-cols-[1fr_410px] gap-3">
    <Panel title="Menu" sub={shown.length+' items available'}>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2.5 sm:grid-cols-3 2xl:grid-cols-4">{shown.map(p=><button key={p.id} onClick={()=>add(p)} className="text-left rounded-xl border border-slate-200 bg-white p-4 hover:border-[#22A53A] hover:transition-colors"><div className="grid h-20 place-items-center rounded-lg bg-slate-50"><UtensilsCrossed className="text-slate-300" size={28}/></div><b className="block mt-3 line-clamp-2">{p.name}</b><span className="text-xs text-slate-400">{p.category_name||'Other'}</span><div className="mt-2 text-[14px] font-medium text-[#169B36]">{money(p.resolved_price,currency)}</div></button>)}</div>
    </Panel>
