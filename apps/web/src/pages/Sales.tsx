 ...
 return <div>
  ...
  <Panel title="Recent Sales" sub={rows.length+' transactions'}>
   <DataTable head={['Receipt','Date','Customer','Total','Paid','Due','Refund','Status','Action']} rows={rows.map(s=>[
    ...
    <button onClick={()=>openSale(Number(s.id))} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold"><Eye size={14}/>Open</button>
   ])}/>
  </Panel>
  ...
  {detailOpen&&detail&&<Modal title={'Sale · '+detail.sale.receipt_no} onClose={()=>setDetailOpen(false)}>
    ...
    {!detail.sale.voided&&Number(detail.sale.amount_paid)>Number(detail.sale.refunded_amount)&&<div className="mt-5 grid grid-cols-3 gap-2">
      ...
      <button onClick={()=>openAction('void_sale')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 py-2.5 text-xs font-bold text-red-700"><ShieldAlert size={15}/>Void Sale</button>
    </div>}
    {message&&<div className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{message}</div>}
  </Modal>}
  ...
  {actionOpen&&detail&&<Modal title={mode==='refund'?'Request Partial / Full Refund':mode==='void_item'?'Request Item Void':'Request Full Sale Void'} onClose={()=>setActionOpen(false)}>
    ...
    <label className="mt-3 flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm font-semibold"><input type="checkbox" checked={restock} onChange={e=>setRestock(e.target.checked)}/>Return approved quantities to inventory / recipe stock</label>
    <label className="mt-2 flex items-center gap-3 rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-800"><input type="checkbox" checked={urgent} onChange={e=>setUrgent(e.target.checked)}/>Mark management request as urgent</label>
    {message&&<div className="mt-3 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">{message}</div>}
    <button onClick={submit} disabled={busy||!reason.trim()} className={'mt-4 w-full rounded-xl py-3 font-black text-white disabled:opacity-40 '+(mode==='void_sale'?'bg-red-600':'bg-slate-950')}>{busy?'Submitting…':'Send for Management Approval'}</button>
  </Modal>}
 </div>
}
