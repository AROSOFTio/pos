...
import { Building2, UtensilsCrossed, ReceiptText, Percent, ShieldCheck, ImagePlus, Pencil, Plus, Printer, LayoutGrid } from 'lucide-react'
...
type Section='business'|'tables'|'menu'|'documents'|'tax'|'security'
 ...
 return <div>
  ...
  <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)]">
    <aside className="h-fit overflow-hidden rounded-xl border border-slate-200 bg-white lg:sticky lg:top-[78px]">
      ...
      <nav className="p-2">{nav.map(([id,label,Icon,sub])=><button key={id} onClick={()=>{setSection(id);setMessage('');setError('')}} className={'mb-1 flex w-full items-start gap-3 rounded-lg px-3 py-3 text-left transition '+(section===id?'bg-emerald-50 text-emerald-800':'text-slate-600 hover:bg-slate-50')}>
  ...
  {categoryOpen&&<Modal title="Add Menu Category" onClose={()=>setCategoryOpen(false)}><div className="grid gap-3"><Field label="Category name"><input className="control" value={category.name} onChange={e=>setCategory({...category,name:e.target.value})}/></Field><Field label="Default kitchen station"><select className="control" value={category.defaultStationId} onChange={e=>setCategory({...category,defaultStationId:Number(e.target.value)})}><option value="0">None</option>{stations.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><button onClick={saveCategory} disabled={!category.name.trim()} className="rounded-lg bg-[#22A53A] py-3 text-[12px] font-semibold text-white disabled:opacity-40">Save Category</button></div></Modal>}
