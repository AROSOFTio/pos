...
export default function Settings(){
 ...
 const [menuForm,setMenuForm]=useState({productId:0,categoryId:0,kitchenStationId:0,price:0,available:true,soldOut:false})
 ...
 function openNewMenu(){setEditingMenu(null);setMenuForm({productId:Number(availableProducts[0]?.id||0),categoryId:Number(categories[0]?.id||0),kitchenStationId:0,price:Number(availableProducts[0]?.price||0),available:true,soldOut:false});setMenuOpen(true)}
 function openMenuEdit(x:any){setEditingMenu(x);setMenuForm({productId:Number(x.product_id),categoryId:Number(x.category_id||0),kitchenStationId:Number(x.kitchen_station_id||0),price:Number(x.base_price||0),available:!!x.available,soldOut:!!x.sold_out});setMenuOpen(true)}
 ...
 return <div>
  <PageHeading eyebrow="Management setup" title="Restaurant Settings" sub="Configure the restaurant identity, floors, tables, menu, pricing, kitchen routing, receipts and other management controls."/>
  ...
  <div className="grid gap-4 xl:grid-cols-2">
   <Panel title="Restaurant identity & branding" sub="Restaurant name and identity used throughout the POS and on documents.">
    <div className="mb-4 flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center">
      ...
      <div className="flex-1"><div className="text-[12px] font-semibold text-slate-700">Restaurant logo</div><div className="mt-1 text-[10.5px] leading-4 text-slate-400">Used on receipts, bills, invoices and reports.</div><label className="mt-2 inline-flex cursor-pointer rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-medium text-slate-600 hover:bg-slate-50">Upload logo<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e=>uploadLogo(e.target.files?.[0])}/></label></div>
    </div>
   ...
   <Panel title="Receipt & bill template" sub="Control how running bills and final receipts appear.">
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Field label="Footer / message"><textarea className="control min-h-20" value={s.document_footer||''} onChange={e=>patch('document_footer',e.target.value)} /></Field></div>
    </div>
  ...
  <div className="mt-4 grid gap-4 xl:grid-cols-2">
   <Panel title="Floors / restaurant areas" sub="Create the physical service areas of the restaurant." action={<button onClick={()=>{setArea({...area,branchId:Number(branches[0]?.id||0)});setAreaOpen(true)}} className="rounded-lg border border-slate-200 px-3 py-2 text-[11px] font-medium"><Plus size={13} className="mr-1 inline"/>Add Floor / Area</button>}>
    <DataTable head={['Area / Floor','Branch','Tables']} rows={areas.map(a=>[<b>{a.name}</b>,a.branch_name,a.table_count])}/>
   </Panel>

   <Panel title="Restaurant tables" sub="Manage table names, seats, occupancy and clean/dirty state." action={<button onClick={()=>{setEditingTable(null);setTable({branchId:Number(branches[0]?.id||0),areaId:0,name:'',code:'',capacity:2,status:'available',cleanlinessStatus:'clean'});setTableOpen(true)}} className="rounded-lg bg-slate-950 px-3 py-2 text-[11px] font-medium text-white"><Plus size={13} className="mr-1 inline"/>Add Table</button>}>
     <DataTable head={['Table','Floor','Seats','Occupancy','Cleanliness','']} rows={tables.map(t=>[
       <b>{t.name}</b>,t.area_name||'Main Floor',t.capacity,
       <Badge tone={t.status==='occupied'?'red':t.status==='reserved'?'amber':'green'}>{t.status==='available'?'vacant':t.status}</Badge>,
       <Badge tone={t.cleanliness_status==='dirty'?'red':'green'}>{t.cleanliness_status||'clean'}</Badge>,
       <button onClick={()=>openTableEdit(t)} className="inline-flex items-center gap-1 text-[11px] font-medium text-slate-600"><Pencil size={12}/>Edit</button>
     ])}/>
   </Panel>
  </div>

  <div className="mt-4 grid gap-4 xl:grid-cols-[.7fr_1.3fr]">
   <Panel title="Menu categories" sub="Breakfast, Meals, Drinks, Desserts, Bar and other menu groups." action={<button onClick={()=>setCategoryOpen(true)} className="rounded-lg border border-slate-200 px-3 py-2 text-[11px] font-medium"><Plus size={13} className="mr-1 inline"/>Category</button>}>
    <DataTable head={['Category','Kitchen station']} rows={categories.map(c=>[<b>{c.name}</b>,c.station_name||'-'])}/>
   </Panel>

   <Panel title="Menu & pricing" sub="Set what appears on the restaurant menu, its category, price, kitchen route and availability." action={<button onClick={openNewMenu} disabled={!availableProducts.length} className="rounded-lg bg-slate-950 px-3 py-2 text-[11px] font-medium text-white disabled:opacity-40"><Plus size={13} className="mr-1 inline"/>Menu Item</button>}>
    <DataTable head={['Menu item','Category','Station','Price','Availability','']} rows={menu.map(x=>[
      <b>{x.product_name}</b>,x.category_name||'Other',x.station_name||'-',money(x.base_price,currency),
      x.sold_out?<Badge tone="red">Sold out</Badge>:x.available?<Badge tone="green">Available</Badge>:<Badge>Hidden</Badge>,
      <button onClick={()=>openMenuEdit(x)} className="inline-flex items-center gap-1 text-[11px] font-medium text-slate-600"><Pencil size={12}/>Edit</button>
    ])}/>
   </Panel>
  </div>
  ...
  <div className="mt-4 grid gap-4 xl:grid-cols-2">
   ...
   <Panel title="Kitchen & printing" sub="Create receipt/KOT printer profiles and assign kitchen stations.">
    <div className="grid gap-3 sm:grid-cols-2">
  ...
  <div className="mt-4"><Panel title="Printer profiles"><DataTable head={['Profile','Document','Paper','Printer','Station','Status']} rows={profiles.map(p=>[p.name,p.document_type,p.paper_size,p.printer_name||'Browser / PDF',p.station_name||'-',<Badge tone={p.active?'green':'red'}>{p.active?'Active':'Inactive'}</Badge>])}/></Panel></div>
  ...
  {areaOpen&&<Modal title="Add Floor / Area" onClose={()=>setAreaOpen(false)}><div className="grid gap-3"><Field label="Branch"><select className="control" value={area.branchId} onChange={e=>setArea({...area,branchId:Number(e.target.value)})}><option value="0">Choose branch</option>{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></Field><Field label="Floor / area name"><input className="control" value={area.name} onChange={e=>setArea({...area,name:e.target.value})} placeholder="Main Floor, Terrace, VIP"/></Field><button onClick={addArea} disabled={!area.branchId||!area.name.trim()} className="rounded-lg bg-[#22A53A] py-3 text-[12px] font-semibold text-white disabled:opacity-40">Save Floor / Area</button></div></Modal>}

  {tableOpen&&<Modal title={editingTable?'Edit Table':'Add Table'} onClose={()=>setTableOpen(false)} size="lg"><div className="grid gap-3 sm:grid-cols-2">
  ...
  </div><button onClick={saveTable} disabled={!table.branchId||!table.name.trim()} className="mt-4 w-full rounded-lg bg-[#22A53A] py-3 text-[12px] font-semibold text-white disabled:opacity-40">Save Table</button></Modal>}

  {categoryOpen&&<Modal title="Add Menu Category" onClose={()=>setCategoryOpen(false)}><div className="grid gap-3"><Field label="Category name"><input className="control" value={category.name} onChange={e=>setCategory({...category,name:e.target.value})} placeholder="Breakfast, Main Meals, Drinks…"/></Field><Field label="Default kitchen station"><select className="control" value={category.defaultStationId} onChange={e=>setCategory({...category,defaultStationId:Number(e.target.value)})}><option value="0">None</option>{stations.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><button onClick={saveCategory} disabled={!category.name.trim()} className="rounded-lg bg-[#22A53A] py-3 text-[12px] font-semibold text-white disabled:opacity-40">Save Category</button></div></Modal>}

  {menuOpen&&<Modal title={editingMenu?'Edit Menu Item':'Add Menu Item'} onClose={()=>setMenuOpen(false)} size="lg"><div className="grid gap-3 sm:grid-cols-2">
   <Field label="Product / item"><select className="control" value={menuForm.productId} disabled={!!editingMenu} onChange={e=>{const id=Number(e.target.value),p=products.find(x=>Number(x.id)===id);setMenuForm({...menuForm,productId:id,price:Number(p?.price||0)})}}><option value="0">Choose product</option>{availableProducts.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
   ...
   <Field label="Selling price"><input className="control" type="number" min="0" step="0.01" value={menuForm.price} onChange={e=>setMenuForm({...menuForm,price:Number(e.target.value)})}/></Field>
  ...
  <div className="mt-3 flex flex-wrap gap-4"><label className="flex items-center gap-2 text-[12px] text-slate-600"><input type="checkbox" checked={menuForm.available} onChange={e=>setMenuForm({...menuForm,available:e.target.checked})} className="accent-[#22A53A]"/>Available on menu</label><label className="flex items-center gap-2 text-[12px] text-slate-600"><input type="checkbox" checked={menuForm.soldOut} onChange={e=>setMenuForm({...menuForm,soldOut:e.target.checked})} className="accent-red-500"/>Sold out</label></div>
  <button onClick={saveMenu} disabled={!menuForm.productId} className="mt-4 w-full rounded-lg bg-[#22A53A] py-3 text-[12px] font-semibold text-white disabled:opacity-40">Save Menu Item</button></Modal>}
 </div>
}
