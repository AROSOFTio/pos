...
export default function Supermarket({currency}:{currency:string}){
 ...
 return <div>
  ...
  <div className="grid xl:grid-cols-[1fr_410px] gap-3">
    <Panel title="Products" sub={shown.length+' items available'}>
  ...
  {requestOpen&&<Modal title="Request New Product" onClose={()=>!requestBusy&&setRequestOpen(false)} size="sm">
    ...
    <button onClick={submitRequest} disabled={requestBusy||(!requestName.trim()&&!requestCode.trim())} className="mt-4 w-full rounded-lg bg-[var(--brand-primary)] py-3 text-[12px] font-semibold text-white disabled:opacity-40">{requestBusy?'Sending…':'Send Product Request'}</button>
  </Modal>}
 </div>
}
