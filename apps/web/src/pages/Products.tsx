...
export default function Products({currency}:{currency:string}){
 ...
 return <div><PageHeading eyebrow="Catalogue" title="Products" sub="Sellable products, prices, stock thresholds and supplier links." action={<button onClick={show} className="rounded-xl bg-slate-950 text-white px-4 py-2.5 text-sm font-bold"><Plus size={16} className="inline mr-1"/>New Product</button>}/>
 ...
 </div><div className="mt-4"><div className="text-sm font-semibold text-slate-700">Suppliers</div><div className="mt-2 grid sm:grid-cols-2 gap-2 max-h-36 overflow-y-auto">{suppliers.map(s=><label key={s.id} className="flex items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm"><input type="checkbox" checked={form.supplierIds.includes(Number(s.id))} onChange={e=>setForm({...form,supplierIds:e.target.checked?[...form.supplierIds,Number(s.id)]:form.supplierIds.filter(id=>id!==Number(s.id))})}/>{s.name}</label>)}</div></div><button onClick={save} className="mt-4 w-full rounded-xl bg-slate-950 text-white py-3 font-bold">Save Product</button></Modal>}</div>
}
