...
export default function Branches(){
 ...
 return <div>
  <PageHeading eyebrow="Organisation" title="Branches & Terminals" sub="Business locations, stock stores and POS terminals." action={<div className="flex gap-2"><button onClick={()=>setTerminalOpen(true)} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium"><Monitor size={15} className="mr-1 inline"/>Terminal</button><button onClick={()=>setOpen(true)} className="rounded-xl bg-slate-950 px-4 py-2.5 text-[13px] font-medium text-white"><Plus size={15} className="mr-1 inline"/>Branch</button></div>}/>
