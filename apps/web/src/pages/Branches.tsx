...
export default function Branches(){
 ...
 return <div>
  ...
  <div className="grid gap-4 xl:grid-cols-2">
    <Panel title="Branches"><DataTable head={['Branch','Location','Status']} rows={rows.map(x=>[<div className="flex items-center gap-2"><Building2 size={15}/><span>{x.name}</span></div>,x.location||'-',<Badge tone={x.active?'green':'red'}>{x.active?'Active':'Inactive'}</Badge>])}/></Panel>
    <Panel title="Terminals"><DataTable head={['Terminal','Code','Branch','Status']} rows={terminals.map(x=>[<div className="flex items-center gap-2"><Monitor size={15}/><span>{x.name}</span></div>,x.code,x.branch_name,<Badge tone={x.active?'green':'red'}>{x.active?'Active':'Inactive'}</Badge>])}/></Panel>
  </div>
