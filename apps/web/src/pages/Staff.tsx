...
export default function Staff(){
 const [rows,setRows]=useState<any[]|null>(null),[branches,setBranches]=useState<any[]>([]),[permissions,setPermissions]=useState<any[]>([]),[timeRows,setTimeRows]=useState<any[]>([]),[section,setSection]=useState<'staff'|'permissions'|'attendance'>('staff')
 ...
 const load=()=>Promise.all([api('/staff'),api('/branches'),api('/permissions'),api('/staff/timeclock').catch(()=>[])]).then(([u,b,p,t])=>{setRows(u);setBranches(b);setPermissions(p);setTimeRows(Array.isArray(t)?t:[])})
 useEffect(()=>{load()},[])
