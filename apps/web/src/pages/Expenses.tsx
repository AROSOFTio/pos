   ...
   {open&&<Modal title={preview?'Preview Expense Voucher':'Record Expense'} onClose={()=>setOpen(false)}>
     {!preview?<div className="space-y-4">
       <div className="grid sm:grid-cols-2 gap-3">
         <label className="text-[13px] font-medium text-slate-700">Category<input value={category} onChange={e=>setCategory(e.target.value)} className="control"/></label>
         <label className="text-[13px] font-medium text-slate-700">Date<input type="date" value={date} onChange={e=>setDate(e.target.value)} className="control"/></label>
       </div>
       <label className="text-[13px] font-medium text-slate-700">Description<textarea value={description} onChange={e=>setDescription(e.target.value)} className="control min-h-24" placeholder="What was this expense for?"/></label>
       <label className="text-[13px] font-medium text-slate-700">Amount<input type="number" min="0" step="0.01" value={amount||''} onChange={e=>setAmount(Number(e.target.value))} className="control"/></label>
       <div className="flex justify-end gap-2 pt-2"><button onClick={()=>setPreview(true)} disabled={!description.trim()||!(amount>0)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium disabled:opacity-40"><Eye size={15} className="inline mr-1"/>Preview</button><button onClick={save} disabled={saving||!description.trim()||!(amount>0)} className="rounded-xl bg-slate-900 text-white px-4 py-2.5 text-sm font-medium disabled:opacity-40">{saving?'Saving…':'Save Expense'}</button></div>
     </div>:<div>
       <div className="rounded-xl border border-slate-200 overflow-hidden">
         <div className="bg-slate-900 text-white p-5"><div className="text-xs text-slate-400">EXPENSE / COST VOUCHER PREVIEW</div><div className="font-semibold text-lg mt-1">Unsaved Expense Voucher</div></div>
