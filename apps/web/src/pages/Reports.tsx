...
export default function Reports({currency}:{currency:string}){
  const [summary,setSummary]=useState<any|null>(null),[from,setFrom]=useState(''),[to,setTo]=useState(''),[type,setType]=useState('financial'),[busy,setBusy]=useState('')
  const qs=useMemo(()=>new URLSearchParams({...(from?{from}:{}),...(to?{to}:{})}).toString(),[from,to])
  const load=()=>api('/reports/summary'+(qs?'?'+qs:'')).then(setSummary)
  useEffect(()=>{load()},[qs])
  async function exp(format:'xlsx'|'csv'|'pdf'){
    setBusy(format)
    try{
      const p='/reports/export?report='+encodeURIComponent(type)+(from?'&from='+from:'')+(to?'&to='+to:'')+'&format='+format
  ...
  return <div>
    ...
    <div className="mt-4"><Panel title="Report Centre" sub="XLSX is a real Excel workbook. CSV and PDF are available too.">
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <label className="text-sm font-medium text-slate-600">To<input type="date" className="control" value={to} onChange={e=>setTo(e.target.value)}/></label>
