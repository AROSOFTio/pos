...
export default function Purchasing({currency}:{currency:string}){
 ...
 if(!o)return <Loading/>
 return <div>
  <PageHeading eyebrow="Procurement" title="Purchasing & Receiving" sub="Supplier-controlled purchase orders, approval status and goods receiving." action={<button onClick={openCreate} className="rounded-xl bg-slate-950 text-white px-4 py-2.5 text-sm font-bold"><Plus size={16} className="inline mr-1"/>New Purchase Order</button>}/>
  ...
  <div className="grid xl:grid-cols-[1.3fr_.7fr] gap-4 mt-4">
    <Panel title="Purchase Orders"><DataTable head={['PO','Supplier','Status','Received','Total','Actions']} rows={pos.map(x=>[
      ...
      <div className="flex gap-2">
        {(x.status==='draft'||x.status==='rejected')&&<button onClick={()=>setSubmitPo(x)} className="rounded-lg bg-slate-950 text-white px-3 py-1.5 text-xs font-bold">{x.status==='rejected'?'Resubmit':'Submit'}</button>}
  ...
  {createOpen&&<Modal title={preview?'Preview Purchase Order':'New Purchase Order'} onClose={()=>setCreateOpen(false)}>
    {!preview?<div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        <label className="text-sm font-semibold text-slate-700">Supplier<select value={supplierId} onChange={e=>chooseSupplier(Number(e.target.value))} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5"><option value="0">Choose supplier</option>{suppliers.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
        <label className="text-sm font-semibold text-slate-700">Branch<select value={branchId} onChange={e=>setBranchId(Number(e.target.value))} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5">{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label>
      </div>
      <div className="rounded-xl bg-slate-50 p-3">
        <div className="flex gap-2">
          ...
          <button onClick={addProduct} className="rounded-lg bg-slate-950 text-white px-4 text-sm font-bold">Add</button>
        </div>
      ...
      <div className="flex justify-between items-center border-t border-slate-100 pt-4"><div><div className="text-xs text-slate-400">Draft total</div><div className="text-xl font-black">{money(total,currency)}</div></div><div className="flex gap-2"><button onClick={()=>setPreview(true)} disabled={!supplierId||!items.length} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold disabled:opacity-40"><Eye size={15} className="inline mr-1"/>Preview</button><button onClick={savePO} disabled={saving||!supplierId||!items.length} className="rounded-xl bg-slate-950 text-white px-4 py-2.5 text-sm font-bold disabled:opacity-40">{saving?'Saving…':'Save Draft'}</button></div></div>
    </div>:<div>
      <div className="rounded-2xl border border-slate-200 overflow-hidden">
        <div className="bg-slate-950 text-white p-5 flex justify-between"><div><div className="text-xs text-slate-400">PURCHASE ORDER PREVIEW</div><div className="font-black text-lg mt-1">Draft Purchase Order</div></div><div className="text-right text-sm"><div>{suppliers.find(s=>Number(s.id)===supplierId)?.name}</div><div className="text-slate-400">{branches.find(b=>Number(b.id)===branchId)?.name}</div></div></div>
        <div className="p-5"><DataTable head={['Product','Qty','Unit Cost','Total']} rows={items.map(x=>[<b>{x.name}</b>,x.qty,money(x.unitCost,currency),money(x.qty*x.unitCost,currency)])}/><div className="mt-4 text-right"><div className="text-xs text-slate-400">TOTAL</div><div className="text-2xl font-black">{money(total,currency)}</div></div>{notes&&<div className="mt-4 text-sm text-slate-500">{notes}</div>}</div>
      </div>
      <div className="mt-4 flex justify-end gap-2"><button onClick={()=>setPreview(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold">Back</button><button onClick={savePO} disabled={saving} className="rounded-xl bg-slate-950 text-white px-4 py-2.5 text-sm font-bold">{saving?'Saving…':'Save Draft PO'}</button></div>
    </div>}
  </Modal>}
  ...
  {submitPo&&<Modal title={(submitPo.status==='rejected'?'Resubmit ':'Submit ')+submitPo.po_no+' for Approval'} onClose={()=>setSubmitPo(null)}>
    ...
    <label className="mt-3 flex items-center gap-3 rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-800"><input type="checkbox" checked={urgent} onChange={e=>setUrgent(e.target.checked)}/>Mark as urgent</label>
    <button onClick={sendApproval} className="mt-4 w-full rounded-xl bg-slate-950 text-white py-3 font-bold">Submit for Approval</button>
  </Modal>}
 </div>
}
